...
Before a non-dry-run dispatch, `start` reads the workspace webhook configuration and refuses to post unless an enabled `Comment` webhook covers the primary ticket's team directly or through `allPublicTeams: true`. A coverage gap names the uncovered team; a read or permission failure is reported separately. The Linear credential therefore needs workspace-admin webhook read access. Bundles check the primary team because only the primary receives the dispatch comment. `--dry-run` does not perform this live coverage check.
