...
async def _repo_config_for_schedule(full_name: str | None) -> dict[str, str] | None:
...
async def create_agent_schedule(
    ...
    repo = await _repo_config_for_schedule(body.repo)
...
async def update_agent_schedule(
    ...
    if body.repo is not None:
        patch["repo"] = await _repo_config_for_schedule(body.repo)
...
async def launch_scheduled_agent_run(schedule_id: str) -> dict[str, Any]:
    ...
    if full_name:
        ...
        try:
            await _repo_config_for_schedule(full_name)
